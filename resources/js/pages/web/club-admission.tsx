...
import { AdmissionDetails } from '@/types';
import { Head } from '@inertiajs/react';
import { useEffect, useState } from 'react';

export default function mission() {
    const [admissionDetails, setAdmissionDetails] = useState<AdmissionDetails | null>(null);

    useEffect(() => {
        import('axios').then(({ default: axios }) => {
            axios
                .get('/storage/club_admission_details.json', { headers: { 'Cache-Control': 'no-store' } })
                .then((res) => setAdmissionDetails(res.data))
                    ...
                    <div className="flex w-full flex-col items-center justify-between gap-5 lg:grid-cols-2 lg:flex-row">
                        <div className="flex w-full flex-col items-center justify-center gap-5 lg:items-start">
                            <div className="flex flex-col items-center justify-center gap-2 lg:items-start">
                                <h2 className="text-center text-2xl font-semibold md:text-4xl xl:text-3xl">Admission details</h2>
                            </div>

                            {admissionDetails?.admission_fee ? (
                                <p>
                                    Club admission fee <span className="text-primary/80 font-bold">{admissionDetails?.admission_fee}Tk.</span>
                                </p>
                            ) : null}

                            <ul className="font-bangla text-primary/80 list-decimal space-y-2 pl-5 text-justify text-sm sm:text-base">
                                {Array.isArray(admissionDetails?.admission_rules) && admissionDetails.admission_rules.length > 0
                                    ? admissionDetails.admission_rules.map((admissionRule, idx) => (
