...
export default function home({ siteDetails, events }: { siteDetails: siteConfig; events: Event[] }) {
    ...
    const upcomingEvents = events
        ?.filter((event) => !event.end || new Date(event.end) > now) // Not ended yet or no end date
        .sort((a, b) => {
            const aStart = new Date(a.start);
            const aEnd = a.end ? new Date(a.end) : null;
            const bStart = new Date(b.start);
            const bEnd = b.end ? new Date(b.end) : null;

            const aOngoing = aStart <= now && (!aEnd || aEnd > now);
            const bOngoing = bStart <= now && (!bEnd || bEnd > now);

            if (aOngoing && !bOngoing) return -1;
            if (!aOngoing && bOngoing) return 1;
            // If both are ongoing or both are not, sort by start date
                ...
                <section className="flex flex-col items-center justify-center gap-10">
                    ...
                    <div className="flex w-full flex-wrap items-center justify-center gap-5">
                        {upcomingEvents && upcomingEvents.length > 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center gap-2">
                                <p className="text-primary/80 text-center text-base font-medium">No events available at the moment.</p>
                            </div>
                        )}
                    </div>
                </section>
